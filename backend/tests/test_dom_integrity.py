...
def test_3_price_coordinate_normalization():
...
def test_4_bid_ask_grid_alignment():
    p = 4335.20
    step = 8.67
...
def test_5_source_weight_redistribution():
    weights_all = renormalize_weights(["MT5", "CTRADER"])
    assert sum(weights_all.values()) == pytest.approx(1.0)
    assert weights_all["MT5"] == pytest.approx(0.50)
    assert weights_all["CTRADER"] == pytest.approx(0.50)

    weights_single = renormalize_weights(["MT5"])
    assert weights_single["MT5"] == pytest.approx(1.0)


def test_6_volume_relative_scoring():
    levels = [
...
@pytest.mark.asyncio
async def test_7_bid_ask_semantics_validation():
    engine = DOMEngine()
    now = time.time()
    mock_mt5 = AsyncMock()
    mock_mt5.source_id = "MT5"
    mock_mt5.source_name = "MetaTrader 5"
    ...
    with patch.object(engine.adapters[0], "fetch_snapshot", new=mock_mt5.fetch_snapshot):
...
def test_8_proximity_scoring_cases():
    engine = DOMEngine()
    p = 4335.20
    step = 8.67
    ...
    assert top_b == "4335.00–4343.67"


@pytest.mark.asyncio
async def test_9_realistic_xauusd_scenario():
    engine = DOMEngine()
    now = time.time()
    ...
    mock_mt5.fetch_snapshot = AsyncMock(return_value=SourceSnapshot(
        ...
        observed_timestamp=now,
        raw_spot_price=4335.20,
        bids=[PriceLevel(price=4330.00, volume=600.0)],
    ...
    mock_ctrader.source_name = "cTrader Open API"
    mock_ctrader.fetch_snapshot = AsyncMock(return_value=SourceSnapshot(
        ...
        observed_timestamp=now,
        raw_spot_price=4335.20,
        bids=[PriceLevel(price=4331.00, volume=500.0)],
        asks=[PriceLevel(price=4336.20, volume=800.0)],
        included_in_aggregation=True
    ))

    with patch.object(engine.adapters[0], "fetch_snapshot", new=mock_mt5.fetch_snapshot):
        with patch.object(engine.adapters[1], "fetch_snapshot", new=mock_ctrader.fetch_snapshot):
            dom_intel = await engine.build_dom_intelligence("XAUUSD", current_price=4335.20)
            assert dom_intel.current_price == pytest.approx(4335.20)
            assert len(dom_intel.liquidity) > 0
            assert dom_intel.liquidity[0].observed is True
            assert "MULTI-SOURCE (2/2)" in dom_intel.coverage
